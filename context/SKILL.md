...
version: 0.1.0
description: >
  Captures or refreshes a project's Design Bible by reading local design tokens,
  asking a short brand interview, and writing rules that other designStack skills
...
**If `DESIGN_BIBLE` is `yes` and `BIBLE_PATH` is not `GIT_ROOT/design/DESIGN-BIBLE.md`:** Read the existing file at `BIBLE_PATH` fully. Note everything — colors, fonts, spacing, component rules. You will extend, not replace it. Tell the user:
...
Read `references/design-bible-template.md` and write `design/DESIGN-BIBLE.md` in that structure. Fill every section from what you gathered in Steps 1–3. Use real values — not placeholders. If a value is unknown, write `unknown — update me` rather than leaving a blank.
...
- If no: create it by copying `../AGENTS.md` as a template, then fill in the design rules block.
