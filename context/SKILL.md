...
> "I don't see a project here. Make sure you open Claude Code from your project folder — the one that has your `src/` or `package.json`. Then run `/designStack:context` again."
...
```markdown
# Design Bible
> Generated by designStack v0.1.0 on [date]. Updated automatically by /designStack:look and /designStack:context.
...
[Populated over time by /designStack:look scans. Empty to start.]

## Gap Audit
- Screens not yet reviewed: [list or "all — run /designStack:look on each"]
...
Source: dstack/DESIGN-BIBLE.md — run /designStack:context to refresh
...
If yes, run the /designStack:save flow with commit message: "Set up Design Bible with [3 vibe words] brand rules".
